...
TARGET		:=	newcode
BUILD		:=	build
SOURCES		:=	source source/asm source/misc source/actor
INCLUDES	:=	include source/misc
DATA		:=	data  
GRAPHICS	:=	gfx  
...
CFLAGS	:=	-g -Wall -Wno-parentheses -Wno-unknown-pragmas -Wno-unused-function -Wno-multichar -Wno-comment -Wno-register \
 		-march=armv5te -mtune=arm946e-s -fomit-frame-pointer\
		-ffast-math \
		$(ARCH)

CFLAGS	+=	$(INCLUDE) -DSDK_GCC -DSDK_CW -DSDK_ARM9 -DARM9 -DSDK_CODE_ARM -fno-threadsafe-statics
CXXFLAGS	:= $(CFLAGS) -fno-rtti -fno-exceptions -std=c++17

ASFLAGS	:=	-g -march=armv5te -mtune=arm946e-s
LDFLAGS	=	-T $(CURDIR)/../symbols9.x -T $(CURDIR)/../linker.x -g $(ARCH) -Map newcode.map
